...
namespace PjonkGooseEnemy.ScrapStuff;
public class GoldenEgg : GrabbableObject {
    ...
    private IEnumerator RandomChanceOfHatching() {
        while (mommyAlive && !spawnedOne && !this.isInShipRoom && !this.isHeldByEnemy) {
            ...
            if (UnityEngine.Random.Range(1f, 100f) <= 2.5f*motherlyFactor) {
                ...
                var newGoose = RoundManager.Instance.SpawnEnemyGameObject(RoundManager.Instance.GetRandomNavMeshPositionInRadiusSpherical(this.transform.position, 5f), this.transform.rotation.y, -1, Plugin.PjonkGooseEnemyType);
